...
> 本 skill 的所有"问卷式输入点"都**直接内联**调用 `deep_research`（深度研究）+ 飞书 MCP `sunny_ai__feishu_create_doc`（创建飞书云文档）。每个输入点只提供差异参数（topic / scenario / 飞书命名 / 章节结构），其余动作按下列协议执行。

### 步骤 A · 调用 `deep_research`（必做，不可跳过）

按差异参数中给定的 `research_query` 执行一次 deep_research。若 deep_research 不可用，降级为 WebSearch：**至少 3 次**（中文关键词 1-2 次 + 英文关键词 1-2 次，分开搜索），确保中英文方法论来源均被覆盖。
...
对每个维度**执行内联问卷生成协议**，差异参数见下方「维度参数表」。

⚠️ **收集顺序必须按第二轮「最关键的准备度维度」排序执行，不使用默认 ①-⑥ 顺序。** 例如场景识别判定数据准备度最关键，则先跑 ② 再跑其他维度。每维话术中的"第 N/6 步"按实际执行序号填写。
...
**用户确认后**：
- 继续 → **立即** Read `sunny-ai-project/skills/goal-definition/SKILL.md` 并按其中指令执行，不要等用户再次确认
- 跳过 → **立即** Read `sunny-ai-project/skills/readiness-diagnosis/SKILL.md` 并按其中指令执行，不要等用户再次确认

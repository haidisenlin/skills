...
本 skill 所有"问卷式输入点"**直接内联**调用 `deep_research`（深度研究）+ 飞书 MCP `sunny_ai__feishu_create_doc`（创建飞书云文档）——**AI 主动为每维生成模板飞书文档**给用户填，而非让用户自己新建空文档。
...
六维准备度必须**针对具体 AI 场景**评估，不能脱离场景空谈。开始前先检查上下文：

### 情况 A：从 scene-discovery / scene-prediagnosis / goal-definition 衔接而来

上下文中已有场景确认描述（BU/厂区/部门/改善对象/核心痛点/AI 设想等）。**直接使用**，跳到「维度选择」。

### 情况 B：用户从 initiate 直接进入，或上下文中无场景信息

需要先收集场景基本信息。调用飞书 MCP `sunny_ai__feishu_create_doc`（创建飞书云文档）生成一份场景速描表，内容如下：

```
📋 AI 场景速描（准备度评估前置输入）

请填写以下信息，帮助 AI 针对性地生成六维评估问卷。
...
2. 你想用 AI 改善什么
   • 改善对象：（一句话，如"AA 对准工序良率"）
   • 核心痛点：（如"FPY 87%，客诉 PPM 高"）
   • 对 AI 的初步设想：（如"用视觉 AI 替代人工目检"）

3. 项目现状
   • 是否已有 Sponsor / 领导支持：（是/否/不确定）
   • 是否有预算：（有/正在申请/无）
   • 之前是否尝试过其他改善方案：（是/否，简述）
```

发送引导语：

> 在开始六维准备度评估之前，我需要先了解你的 AI 场景。
> 我生成了一份场景速描表，请花 5 分钟填写：
>
> 📄 {飞书文档链接}
>
> 如果你已经有场景描述（来自之前的场景发现或预诊断），直接粘贴过来也行。

等用户提交后，提取关键槽位（BU/厂区/部门/改善对象/痛点/AI 设想），然后进入「维度选择」。
...
- 选 1 / 直接贴资料 → **立即** Read `sunny-ai-project/skills/readiness-diagnosis/SKILL.md` 并按其中指令执行，不要等用户再次确认
